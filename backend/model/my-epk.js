...
const getSpecialArtistDetails = async (ophid) => {
    ...
    const [rows] = await db.execute("SELECT ud.full_name, pd.PhotoURLs, ud.stage_name, pd.VideoURL, ud.personal_photo, pd.Profession, ud.location, pd.Bio, pd.FacebookLink, pd.InstagramLink, sps.song_name, sps.audio_url, sps.views FROM user_details ud LEFT JOIN professional_details pd on ud.ophid = pd.OPH_ID LEFT JOIN special_artist_songs sps ON ud.ophid = sps.ophid WHERE ud.ophid = ?", [ophid])

    const [song_count] = await db.execute("SELECT ophid, COUNT(ophid) FROM special_artist_songs WHERE ophid = ? AND `status` = 'approved' GROUP BY ophid", [ophid])
