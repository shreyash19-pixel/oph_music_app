...
const addAudioPlatform = async (req, res) => {
  try {
    ...
    const newPlatform = await AudioModel.addAudioPlatform(name.trim());
...
const deleteAudioPlatform = async (req, res) => {
  try {
    ...
    if (affectedRows === 0) {
      return res.status(404).json({ success: false, message: "Audio platform not found" });
    }
...
const updateMetric = async (req, res) => {
  try {
    ...
    if (!body.id && !(body.song_id && typeof body.audio_platform_name !== "undefined")) {
      return res.status(400).json({
        success: false,
        error: "Provide id OR provide song_id and audio_platform_name to identify the record",
      });
    }
    ...
    const payload = {
      ...
      song_name: typeof body.song_name !== "undefined" ? body.song_name : undefined,
      audio_platform_name: typeof body.audio_platform_name !== "undefined" ? body.audio_platform_name : undefined,
      audio_platform_streams:
        typeof body.audio_platform_streams !== "undefined" ? Number(body.audio_platform_streams) : undefined,
      audio_platform_revenue:
        typeof body.audio_platform_revenue !== "undefined" ? String(body.audio_platform_revenue) : undefined,
    };

    const updated = await audioPlatform.updateSongAudioMetric(payload);
    if (!updated) {
      return res.status(404).json({ success: false, error: "No record found to update" });
    }
