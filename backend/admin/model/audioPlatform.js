...
const deleteAudioPlatformById = async (id) => {
  const [result] = await db.query("DELETE FROM audio_platforms WHERE id = ?", [id]);
...
const createSongAudioMetric = async (payload) => {
  ...
  const [rows] = await db.execute("SELECT * FROM song_audio_metrics WHERE id = ?", [result.insertId]);
  return rows[0] || null;
};
...
const updateSongAudioMetric = async (payload) => {
  ...
  const allowed = ["OPH_ID", "song_name", "audio_platform_name", "audio_platform_streams", "audio_platform_revenue"];
  const setParts = [];
  const params = [];
  ...
  for (const key of allowed) {
    if (p[key] !== null) {
      ...
      if (key === "audio_platform_streams") {
        const n = typeof p.audio_platform_streams === "number"
          ? p.audio_platform_streams
          : parseInt(p.audio_platform_streams || 0);
  ...
  if (p.id) {
    whereSql = "WHERE id = ?";
    whereParams = [p.id];
  } else if (p.song_id && (typeof p.audio_platform_name !== "undefined" && p.audio_platform_name !== null)) {
    ...
    throw new Error("Missing id or (song_id and audio_platform_name) to update");
  }
  ...
  if (result && result.affectedRows > 0) {
    ...
    if (p.id) {
      const [rows] = await db.execute("SELECT * FROM song_audio_metrics WHERE id = ?", [p.id]);
      return rows[0] || null;
    } else {
      const [rows] = await db.execute(
