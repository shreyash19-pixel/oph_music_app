import axiosApi from '../admin/src/conf/axios.js';
...
async function fetchOphidSongCounts() {
  try {
    const res = await axiosApi.get('/kpi');
    ...
    const artistScores = data.map(entry => {
      const views = entry.total_views; // Simulate views for now
      const score = calculateScore(views, entry.song_count);
      return { name: entry.OPH_ID, score };
    });
