  ...
  const handleFinalAction = async(type) => {
    ...
    if (type === "Reject") {
      const logData = {
        action: "Reject",
        ophId: ophid,
        paymentId: recentPayment ? recentPayment.paymentId : "No payment",
        status: "Rejected",
      ...
      if(submit.status === 200){
        toast.error(`Payment rejected with reason: ${reason || "No reason provided"}`, { duration: 20000 });
      }
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button
              ...
              className={`px-6 py-3 bg-green-600 text-white font-semibold rounded-xl shadow hover:bg-green-700 transition-colors ${
                actionLocked ? "opacity-50 cursor-not-allowed" : ""
              }`}
            >
              Approve Payment
            </button>
